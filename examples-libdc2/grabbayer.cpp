 ...
 * Description:
 *    Get one color image via DMA transfer using libdc1394 and store it 
 *    as a portable pixel map (ppm).  Based on 'grab_gray_image' from libdc1394 
 *    examples.
 *    
 *-------------------------------------------------------------------------
 *     License: LGPL
...
int main( int argc, char *argv[] ) 
{
   ...
   // note: you are getting a MONO8 mode because Bayer tile pattern images	
   // are MONO8 for all non-FORMAT7 video modes.
   // get video modes:
   ...
   // select highest res mode that is MONO8
   ...
   fprintf(stderr,"Waiting for transmission... \n");
  ...
  fprintf( stderr, "Grabbing an image...\n" );
  //  capture one frame
  ...
  uchar_t* pBayerImage = dc1394_capture_get_dma_buffer( camera );
  uchar_t* pRGBImage = new uchar_t[ nCols*nRows*3 ];


  char* filename = "bayer.ppm";
  ...
  fwrite( pRGBImage, 3, nCols*nRows, imagefile );
  fclose( imagefile );
  printf( "wrote: '%s'\n", filename );
